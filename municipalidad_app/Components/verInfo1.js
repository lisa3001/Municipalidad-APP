...
export default function VerInfo1({ navigation }) {
    ...
    useEffect(() => {
        (async () => {
            try {
              ...
              if(res !== null) {
               setName(res.nombre);
               setApellido(res.primerApellido);
               setApellido2(res.segundoApellido);
               setSexo(res.sexo);
               setDateS(res.fechaNacimiento);
               setTipoIdent(res.tipoIdentificacion);
               setNumeroIdent(res.numeroIdentificacion);
               setUser(res);
              }
            } catch(e) {
              console.log(e);
    ...
    const storeData = async (value) => {
        try {
          ...
          await AsyncStorage.setItem('@tempUpdate', jsonValue)
          navigation.navigate('VerInfo2');
        } catch (e) {
          console.log(e);
        }
      }
    ...
    const validarCampos = () => {
        ...
        if(numeroIdent === ""){
            ...
            if(flag === 0){
                user.nombre = name;
                user.primerApellido = apellido;
                user.segundoApellido = apellido2;
                user.sexo = sexo;
                user.fechaNacimiento = dateS;
                user.tipoIdentificacion = tipoIdent;
                user.numeroIdentificacion = numeroIdent;
                storeData(user);
            }
        }
    }
